from flask import Flask, render_template, request , redirect , url_for, flash,session,jsonify,request,current_app, make_response
...
#######################
# @app.route('/api/propertyAPI', methods=['POST'])
# def create_property():
#     data = request.json
...
#     db.session.commit()
#     return jsonify({'id': new_property.id, 'message': 'Booking created'}), 201
############
@login_manager.user_loader
def load_user(user_id):
    if session.get("role") == "admin":
...
###########################
# @app.route('/api/search_properties', methods=['GET'])
# def search_properties_API():
#     location = request.args.get('location')
#     if location:
#         # Search the properties by location
#         properties = Property.query.filter(Property.location.ilike(f'%{location}%')).all()

#         # Convert the properties to a list of dictionaries for JSON response
#         property_list = [
#             {
#                 'id': prop.id,  # Include the property id
...
########################################
# A Protected Route Example (requires JWT Token)
...
def usersignup():
    if request.method == "POST":
        ...
        # Create a new user without the confirmpassword field
        new_user = Userdata(
            name=name,
            email=email,
            phone=phone,
...
def list_property():
    if request.method == 'POST':
        ...
        # ✅ Extract room_types using request.form directly
        room_types = {}
        for key in request.form:
            if key.startswith("room_types["):
                room_type = key[11:-1]  # extract key from 'room_types[AC]'
        ...
        new_property = Property(
            ...
            room_types=room_types,  # stored as JSON
            user_id=user_id
        )
        db.session.add(new_property)
        db.session.commit()
...
#     for prop in properties:
#         # Safely load room_types if stored as string (Text column)
...
#         # Safely parse room_types if stored as JSON string
#         if isinstance(prop.room_types, str):
#             try:
#                 room_types_data = json.loads(prop.room_types)
#             except json.JSONDecodeError:
...
######################################3
# @app.route('/api/all/<int:user_id>', methods=['GET'])
...
class Booking(db.Model):
    ...
    # New field: ForeignKey to Userdata
...
##############################################333
# @app.route('/api/bookingAPI', methods=['GET', 'POST'])
# def bookings():
#     if request.method == 'POST':
#         try:
#             data = request.get_json()

#             # ✅ Extract user_id sent from Django
#             user_id = data.get('user_id')
#             if not user_id:
...
#                 return jsonify({'error': f'Room type {room_type} not found'}), 400
#             if room_types_dict[room_type] < rooms_booked:
#                 return jsonify({'error': f'Not enough {room_type} rooms available'}), 400
#             if prop.rooms_available < rooms_booked:
#                 return jsonify({'error': 'Not enough total rooms available'}), 400
...
###################################################
# @app.route('/api/bookingAPI/<int:booking_id>', methods=['GET'])
...
#     image_url = (
#             url_for('static', filename=f'images/{property_obj.image}', _external=True)
#         )
...
#     'image': image_url  # This assumes `image` is a URL or relative path
# },
...
#############################################
# @app.route('/api/owner/<int:user_id>/bookings', methods=['GET'])
# def get_bookings_by_owner(user_id):
...
#                     'location': prop.location,
#                     'price': str(prop.price),
#                     'image': prop.image  # If it's a URL or path
#                 }
#             }
